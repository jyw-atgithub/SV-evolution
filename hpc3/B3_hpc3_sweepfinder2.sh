...
#now, the sample number (n) is 62*2=124
#the SNPs are folded now, so the values at folded colum is 1
echo -e "position\tx\tn\tfolded" > ${SF2}/snp.background.txt
bcftools view --threads ${nT} -m2 -M2 -v snps -r "2L,2R,3L,3R,4,X,Y" ${SF2}/filled.snp.vcf.gz|\
bcftools query -f '%POS\t%AC\t124\t1\n' >> ${SF2}/snp.background.txt
#Compute empirical frequency spectrum (-f)
SweepFinder2 -f ${SF2}/snp.background.txt ${SF2}/snp.spect.txt
...
##Create another empirical frequency spectrum with 31 samples and only synonymous sites
...
#extract the SNPs in chromosome 2R
echo -e "position\tx\tn\tfolded" > ${SF2}/2R.snp.input.txt
bcftools view --threads ${nT} -m2 -M2 -v snps -r "2R" ${SF2}/filled.snp.vcf.gz|\
bcftools query -f '%POS\t%AC\t124\t1\n' >> ${SF2}/2R.snp.input.txt
#Actual sweep scan
#1000 means devide the chromosome into 1000 windows
SweepFinder2 -l 1000 ${SF2}/2R.snp.input.txt ${SF2}/snp.spect.txt ${SF2}/2R.snp.output.txt
SweepFinder2 -l 1000 ${SF2}/2R.snp.input.txt ${SF2}/snp.syn.spect.txt ${SF2}/2R.snp.output-2.txt
